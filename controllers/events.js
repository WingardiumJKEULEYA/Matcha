...
// Event Visite : Notification
  events.on('user_visit', function (user, visited) {
    pool.getConnection(function( err, connection ) {
      if ( err ) { console.log('err user_visit EVENT'); return ; }
      
      if (user === visited) {
        // Si l'user visite son propre profile ca sert à rien de continuer
        connection.release();
        return ;
      }
      
      // Verifions si le visité n'a pas bloqué le visiteur
      connection.query("SELECT * FROM blocked_users WHERE user = ? AND blocked_target = ?", [ visited, user ], function (err, rows) {
        if (rows.length == 0) {
          // On prend les infos du visiteur pour la notif
